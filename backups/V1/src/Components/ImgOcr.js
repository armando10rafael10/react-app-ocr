...
import AlertMessage from './secundary/AlertMessage';
import './ImgOcr.css';
...
export default function ImgOcr() {
  ...
  const handleChangeFile = (event) => {
    ...
    try {
      ...
      setAlerta("Imagen seleccionada");
      setFileActive(true);
    } catch (e) {
      document.getElementById('textBox_message').textContent = "";//delete text
      setAlerta("No selecciono ninguna imagen");
  ...
  const handleConvertClick = () => {
    if (fileActive === true) {
      ...
      Tesseract.recognize(
        ...
        setAlerta("finalizo el procesamiento de la imagen");
      });
    } else {
      setProgressActive(false);
      setAlerta("No hay una imagen para procesar..!!");
    }
  }
  ...
  const handleCopyClick = () => {
    ...
    try {
      ...
      if (res)
        setAlerta('¡¡Código copiado al portapapeles!!');
      else
        setAlerta('¡¡Ha fallado el copiado al portapapeles!!');
    } catch (ex) {
      setAlerta("Se ha producido un error al copiar al portapaples..");
    }
